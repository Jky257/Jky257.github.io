window.onload=function(){
                ...
                if(checkscrollside()){
                     ...
                     for(var i=0;i<6;i++){ 
                       ...
                       // oImg.src='./images/'+(picNum++)+'.jpg';
						oImg.src='images/pb'+(picNum++)+'.jpg';
                        if(picNum==10){
                            picNum=0;
                            switch (flag){
                                case 0: break;
                                case 1: break;
                                case 2: document.documentElement.scrollTop=0;document.body.scrollTop=0;break;
                            }
                            flag++;
                        };
                    }
		...
		for(var i=0;i<aPin.length;i++){
			...
			if(i<num){
				...
				var minHIndex = getminHIndex(pinHArr,minH);
				 aPin[i].style.position='absolute';//璁剧疆缁濆浣嶇Щ
	...
	// getminHIndex鍑芥暟鑾峰彇鏈€灏忛珮搴in鐨勭储寮�
	function getminHIndex(arr,min){
		for(var i=0;i<arr.length;i++){
           ...
           // checkscrollside鍑芥暟,妫€娴嬫槸鍚︽粴鍔ㄥ簳閮�
